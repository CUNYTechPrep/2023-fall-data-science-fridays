...
# Filename: Ioannou_Georgios_dashboard_iris_callbacks.py
##################################################################################
# Import libraries.
    ...
    if selected_tab == "tab-1":
        ...
        for species in selected_species:
            ...
            fig.add_trace(
                go.Scatter(
                    x=species_df[x_axis],
                    y=[0] * len(species_df),  # All symbols on the same line (y=0)
                    mode="markers",
                    name=species,
                    marker=dict(
            ...
            scatter_plots.append(html.Div(dcc.Graph(figure=fig)))

        return scatter_plots, error_message
        ...
        if len(iris.columns[:-1]) < 3:
            error_message = "Insufficient features for 3D plotting. Please select at least 3 features."
            return html.Div(), error_message
        else:
            fig = go.Figure()

            # Create a dictionary to map species to symbols.

            symbol_map = {"setosa": "circle", "versicolor": "square", "virginica": "x"}

            # Iterate through each species and add a trace with the corresponding symbol.

            for species, symbol in symbol_map.items():
                species_df = subset_df[subset_df["species"] == species]
                trace = go.Scatter3d(
                    x=species_df[x_axis],
                    y=species_df[y_axis],
                    ...
                    name=species,
                    marker=dict(
                        size=marker_size, color=species_colors[species], symbol=symbol
                    ),
                )
                fig.add_trace(trace)
            ...
            fig.update_layout(
                title=f"3D Scatter Plot ({x_axis}, {y_axis}, {z_axis}) for {', '.join(selected_species)}",
                scene=dict(
                    xaxis_title=x_axis,
                    yaxis_title=y_axis,
                    zaxis_title=z_axis,
        ...
        # Calculate and display statistical summaries for selected columns.

        summary_stats = subset_df[selected_columns].describe().reset_index()

        # Display the statistics in a DataTable.
